...
int8_t ICACHE_FLASH_ATTR gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
    ...
    {	
		...
		//下拉菜单事件
        case EVENT_LED_C:
            currentDataPoint.valueLED_C = dataPointPtr->valueLED_C;
            GIZWITS_LOG("Evt: EVENT_LED_C %d\n", currentDataPoint.valueLED_C);
            switch(currentDataPoint.valueLED_C)
            {
            case LED_C_VALUE0: //自定义
                //user handle
                break;
            case LED_C_VALUE1: // 黄色
                //user handle
                break;
            case LED_C_VALUE2:	//紫色
                //user handle
                break;
            case LED_C_VALUE3:	//粉色
                //user handle
                break;
            default:
                break;
