...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLED_C)
    {
        info->event[info->num] = EVENT_LED_C;
        info->num++;
        dataPoints->valueLED_C = gizStandardDecompressionValue(LED_C_BYTEOFFSET,LED_C_BITOFFSET,LED_C_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLED_C != cur->valueLED_C)
    {
        GIZWITS_LOG("valueLED_C Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valueLED_C >= LED_C_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueLED_C Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(LED_C_BYTEOFFSET,LED_C_BITOFFSET,LED_C_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED_C);  
    }
...
void ICACHE_FLASH_ATTR gizwitsInit(void)
{
	...
	//设备属性设置，很多Key或秘钥都是生成SDK时写入到代码中的，所以这些我们都不需要修改
    ...
    attrs.pUserStartFun = devAuthResultCb;
    gagentInit(attrs);
